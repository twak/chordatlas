...
import org.twak.viewTrace.InAxDoubleArray;
...
//		dbg.attachChild( Jme3z.lines( tweed, lines, ColorRGBA.Blue, 2 ) );
	...
	public static Prof retarget( Prof p, SuperLine profileLine ) {
		...
		Prof out =  buildProfile( 
				new Line3d( Pointz.to3( profileLine ) ), 
				l.closestPointOn( p.to3d( p.get( 0 ) ), false ) );
	...
	private static void climb( List<Line3d> lines, Line3d first, Prof monotonic, double max, boolean isUp ) {
		...
		do {
			...
			for ( Line3d l : lines ) {
				...
				if ( ( 
						bt.betterThan ( l.end.y, extrema.y ) || 
						bt.betterThan ( l.start.y, extrema.y ) ) && 
						!ends.containsKey( l ) ) {
	...
	public double distance (Prof other, 
			boolean union, // union or just range of other 
			boolean extendThis,
			boolean extendOther )  // do we extend or penalize when out of range? 
	{
		double meanFit = 0;
		int norm = 0;
		...
		double h     = union ? Math.min ( other.get( 0 ).y, this.get(0).y ) : other.get(0).y,
			   delta = 0.1;
		...
		if (
				get(0).y        > other.get(other.size()-1).y || 
				get(size()-1).y < other.get(0).y)
			return penalty;
		...
		while (true) {
			...
			if ( !extendOther && overO || !extendThis && overT ) 
				...
				Line tl = new Line(       get( t ),       get( t + 1 ) ),
  					 ol = new Line( other.get( o ), other.get( o + 1 ) );
...
//					return MUtils.PI2;
		...
		for (Line l : llines) {
			...
			boolean startAbove = l.start.y >= lastY && l.start.x <= lastX, 
					endAbove   = l.end  .y >= lastY && l.end.x   <= lastX;
	...
	public static Prof parameterize(
			SuperLine profileEdge, // we do not allow profiles to go beyond each profile edge. note than x=0 in a raw profile isn't relative to this edge!
...
//					return MUtils.PI2;
		...
		List<Line> llines = lines.stream().
				filter( l -> l.lengthSquared() > 0.001 ). // is rubbish
...
//			c++;
			...
			if (!(lastL != null && !lastL.isOnLeft( mid ) ||
					(lastP.y == -Double.MAX_VALUE || 
					(mid.y >= lastP.y - 0.5 && mid.x <= lastP.x + 0.5 ) ) ) )
				continue;
			
			boolean 
				startAbove = l.start.y >= lastP.y && l.start.x <= lastP.x, 
				endAbove = l.end.y >= lastP.y && l.end.x <= lastP.x;
...
//			}
			...
			if ( lastL != null && l.start.distanceSquared( lastP ) < 4 ) {
				...
				if ( sec != null 
						&& 
						(sec.distanceSquared( lastP ) < 4 || Math.abs(sec.y - lastP.y) < 1 )
						&& sec.x <= lastL.start.x && sec.y >= lastL.start.y ) 
				{
					clean.remove( clean.size() - 1 );
					clean.add(sec);
					lastP = sec;
...
//		clean.clean( 0.05 );
	...
	public Mesh renderStrip(double width, Point3d point3d) {
		...
		for ( Pair<Point3d, Point3d> p : new ConsecutiveItPairs<>( get3D() ) ) {
			...
			Point3d a1 = new Point3d( p.first () ), a2 = new Point3d (p.first () ),
					b1 = new Point3d( p.second() ), b2 = new Point3d (p.second() );
...
//		out.clean();
	...
	/**
	 * We find an initial base offset. Then we cluster the start point of all (clean) profiles. If
	 * any are a good distance from the initial base, we add those as their own profile lines. 
	 * 
...
//		PaintThing.debug.clear();
		...
		Point2d nStart = superLine.fromPPram( min ),
				nEnd   = superLine.fromPPram( max );
