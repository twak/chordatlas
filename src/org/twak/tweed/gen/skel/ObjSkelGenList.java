...
public class ObjSkelGenList extends Gen implements IDumpObjs, ICanSave {
	...
	*/
	private GlobalUpdate getGlobalUpdate(String fileName) {
		final ObjSkelGen o = objSkelGenMap.get(fileName);

		GlobalUpdate update = new GlobalUpdate() {
			...
			public void run(Class<? extends App> justFinished) {
				...
				tweed.enqueue( new Runnable () {
					...
					public void run() {
						...
						System.out.println("[" + java.time.LocalDate.now() + " " + java.time.LocalTime.now() + "], building " + buildingString + ", net " + justFinished.getName());

						if ( justFinished == PanesLabelApp.class || justFinished == RoofSuperApp.class ) {

							File f = new File ( getFrankenOutput() + buildingString, "out.obj");
							File f_label = new File (getFrankenOutput()  + buildingString + "_labels", "labels.obj");

							if ( justFinished == RoofSuperApp.class ) {
							...
							} else if (justFinished == PanesLabelApp.class ) {
	...
	private String getFrankenOutput() {

		String frankenOutput = Tweed.DATA + File.separator + "franken_out" + File.separator;
		new File (frankenOutput).mkdirs();		
		return frankenOutput;
	}
	...
	private void generateStyles(boolean superRes) {
		...
		for (Map.Entry<String, ObjSkelGen> entry : objSkelGenMap.entrySet()) {
			...
			SelectedApps sa = new SelectedApps( o.blockApp , getGlobalUpdate(filename));
	...
	public JComponent getUI() {
		...
		exemplar = new SelectedApps( objSkelGenMap.get(filename0).blockApp, getGlobalUpdate(filename0));
		BlockApp b =  (BlockApp) exemplar.findRoots().iterator().next();
		b.setJoint(exemplar);
		edit.addActionListener( e -> new JointUI( exemplar, null ).openFrame() );
		ui.add( edit );

//		JButton medium = new JButton( "generate styles (medium)" );  this will not work with the above dump code
//		medium.addActionListener( l -> generateStyles(false) );
//		ui.add( medium );

		JButton styleSuper = new JButton( "dump labels & final (super)" );
