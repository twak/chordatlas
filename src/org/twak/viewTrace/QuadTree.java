...
public abstract class QuadTree<G extends Point2d> {
     ...
     */
    public static class PointRegionQuadTree<P extends Point2d> extends QuadTree<P> {

        private static final Point2d XY_POINT = new Point2d();
        ...
        public PointRegionQuadTree(double x, double y, double width, double height, int leafCapacity, int maxTreeHeight) {
            ...
            AxisAlignedBoundingBox aabb = new AxisAlignedBoundingBox(Point2d,width,height);
        ...
        public boolean insert(double x, double y) {
            ...
            return root.insert((P)Point2d);
        }

        public boolean insert(P pt) {
        	return root.insert(pt);
        }
        ...
        protected static class PointRegionQuadNode<XY extends Point2d> extends QuadNode<XY> {
            ...
            protected void queryRange(AxisAlignedBoundingBox range, List<XY> pointsInRange) {
                ...
                if (isLeaf()) {
                    for (XY Point2d : points) {
                        if (range.containsPoint(Point2d)) 
     ...
     */
    public static class AABBQT<B extends QuadTree.AxisAlignedBoundingBox> extends QuadTree<B> {
         ...
         * @param height Height of the bounding box containing all points
         */
        public AABBQT(double x, double y, double width, double height) {
            this(x,y,width,height,0,0);
        }
        
        public AABBQT(DRectangle r) {
         ...
         */
        public AABBQT(double x, double y, double width, double height, double minWidth, double minHeight) {
            Point2d Point2d = new Point2d(x,y);
            AxisAlignedBoundingBox aabb = new AxisAlignedBoundingBox(Point2d,width,height);
        ...
        public boolean insert(B r) {
        	return root.insert(r);
//            return insert(r.x, r.y,r.getWidth(),r.getHeight());
        }
        ...
        public boolean insert(double x, double y, double width, double height) {
            Point2d Point2d = new Point2d(x,y);
            AxisAlignedBoundingBox range = new AxisAlignedBoundingBox(Point2d,width,height);
        ...
        public Collection<B> queryRange(DRectangle rect) {
        	return queryRange( rect.x, rect.y, rect.width, rect.height );
        }
	...
	public static class QLine extends AxisAlignedBoundingBox {
		public Line line;
		
		public QLine (Line line) {
			
			
			super (new Point2d (Math.min (line.start.x, line.end.x), Math.min (line.start.y, line.end.y)), 
					Math.abs( line.end.x - line.start.x ),
					Math.abs( line.end.y - line.start.y )
					);
			
			if (line.start.equals( line.end ))
				throw new Error("this kills the quad tree!");
			
			this.line = line;
		}
	}

    protected static abstract class QuadNode<G extends Point2d>  {
    ...
    public static class AxisAlignedBoundingBox extends Point2d {
        ...
        public AxisAlignedBoundingBox(Point2d upperLeft, double width, double height) {
    ...
    protected static class TreePrinter {
        ...
        public static <T extends Point2d> String getString(QuadTree<T> tree) {
            if (tree.getRoot() == null) return "Tree has no nodes.";
            return getString(tree.getRoot(), "", true);
        }

        private static <T extends Point2d> String getString(QuadNode<T> node, String prefix, boolean isTail) {
            StringBuilder builder = new StringBuilder();
